...
export default function Collapse({ header, children, motion }: ICollapseProps) {
  ...
  return (
    <div>
      <div onClick={() => setShow(!show)}>{typeof header == "function" ? header?.({ show }) : header}</div>
      <AnimatePresence>{show && <Motion.div {...(motion || defaultMotion)}>{children}</Motion.div>}</AnimatePresence>
    </div>
  )
}
